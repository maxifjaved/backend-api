...
const ensureLoggedIn = async (req, res, next) => {
    ...
    if (!jwtToken) { return res.status(401).json({ message: 'Authentication failed. No token was provided.', errors: { message: 'Authentication failed.' } }) }

    try {
        var decoded = jwt.verify(jwtToken, jwtSecret);
        let nsgUser = await User.getById(decoded.id);
        if (!nsgUser) { return res.status(404).json({ message: 'Sorry, we were unable to find you in our database.', errors: { message: 'Sorry, we were unable to find you in our database. Contact Administrator for more information.' } }) };
